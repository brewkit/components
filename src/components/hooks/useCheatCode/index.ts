...
const defaultCode = ['ArrowUp', 'ArrowUp', 'ArrowDown', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'ArrowLeft', 'ArrowRight', 'KeyB', 'KeyA'];
...
function useCheatCode(callback: () => void, { code = defaultCode } = {}) {
    ...
    const checkMatch = (): void => {
        ...
        if (inputString === codeString || inputString === codeString.replace(/Key./g, 'Tap')) {
            callback
                ? callback()
                : console.log('Cheat code was entered, however no callback was provided.')
        }
    };
